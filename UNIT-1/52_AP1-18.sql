...
DECLARE
    v_raw VARCHAR2(100) := '   mIsHrI   aMRuTIYa   ';
...
BEGIN
    ...
    -- Apply REPLACE twice to collapse multiple spaces.
    ...
    v_first := SUBSTR(
        v_clean, 1, INSTR(v_clean, ' ') - 1
    );

    v_last := SUBSTR(
        v_clean, INSTR(v_clean, ' ') + 1
    );

    v_email := LOWER(
        v_first || '.' || v_last || '@ljku.edu.in'
    );
...
-- OUTPUT:
-- Cleaned Name: Mishri Amrutiya
-- First Name: Mishri
-- Last Name: Amrutiya
-- Email: mishri.amrutiya@ljku.edu.in
-- Length Before: 21
-- Length After: 15